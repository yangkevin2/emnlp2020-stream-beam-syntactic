...
class PhraseTree(object):
    ...
    def pretty_ns(self):
        base = self.pretty(level=0, marker='')
        base = base.replace('\t', ' ')
        base = base.replace('\n', ' ')
        while '  ' in base:
            base = base.replace('  ', ' ')
...
def convert_to_tree(sentence, done_info, fm): # TODO batch this or optimize in the future?
    _, label_actions, all_should_combine = done_info
    # label_actions: seq x 3, all_should_combine: seq x 1
    assert label_actions.size(0) == all_should_combine.size(0)
    i = 0
    stack = []
    for step in range(2 * len(sentence) - 1):
        if all_should_combine[step] == 0:
            stack.append((i, i, [PhraseTree(leaf=i)]))
            i += 1
        else:
            _, right, treelist0 = stack.pop()
            left, _, treelist1 = stack.pop()
            stack.append((left, right, treelist1 + treelist0))
        label = fm.l_action(label_actions[step, 2])
        if label == 'none':
            continue
        for nt in label[6:].split('-'):
            left, right, trees = stack.pop()
