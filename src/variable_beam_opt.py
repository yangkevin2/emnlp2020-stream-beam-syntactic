...
def parse_batch_variable_beam_opt(sentences, fm, network, k=5, ap=2.5, mc=3): # beam decoding
    ...
    for step in range(2 * max_length - 1):
        ...
        valid_label_scores, _ = my_predict_label(network,
                                        fwd.unsqueeze(2).repeat(1, 1, k, 1).flatten(1, 2)[:, valid_beam_indices, :],
                                        back.unsqueeze(2).repeat(1, 1, k, 1).flatten(1, 2)[:, valid_beam_indices, :],
                                        stack[valid_beam_indices],
